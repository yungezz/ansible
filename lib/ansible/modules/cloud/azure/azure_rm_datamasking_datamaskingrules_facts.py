...
DOCUMENTATION = '''
---
module: azure_rm_datamasking_datamaskingrules_facts
version_added: "2.5"
short_description: Get DataMaskingRules facts.
description:
    - Get facts of DataMaskingRules.
...
class AzureRMDataMaskingRulesFacts(AzureRMModuleBase):
    ...
    def list_by_database(self):
        '''
        Gets facts of the specified DataMaskingRules.

        :return: deserialized DataMaskingRulesinstance state dictionary
        '''
        self.log("Checking if the DataMaskingRules instance {0} is present".format(self.))
        found = False
        try:
            response = self.mgmt_client.data_masking_rules.list_by_database(self.resource_group_name,
                                                                            self.server_name,
                                                                            self.database_name,
