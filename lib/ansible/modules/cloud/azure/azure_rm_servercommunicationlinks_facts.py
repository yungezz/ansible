...
EXAMPLES = '''
      - name: Create sample ServerCommunicationLinks
...
class AzureRMServerCommunicationLinksFacts(AzureRMModuleBase):
    def __init__(self):
        ...
        super(AzureRMServerCommunicationLinksFacts, self).__init__(self.module_arg_spec)
    ...
    def get(self):
        ...
        '''
        self.log("Checking if the ServerCommunicationLinks instance {0} is present".format(self.resource_group_name))
        found = False
        try:
    ...
    def list_by_server(self):
        ...
        found = False
        try:
            response = self.mgmt_client.server_communication_links.list_by_server(self.resource_group_name,
                                                                                  self.server_name)
            found = True
            self.log("Response : {0}".format(response))
            self.log("ServerCommunicationLinks instance : {0} found".format(response.name))
...
def main():
    AzureRMServerCommunicationLinksFacts()
