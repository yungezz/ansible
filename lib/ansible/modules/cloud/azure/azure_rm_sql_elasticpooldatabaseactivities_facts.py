...
DOCUMENTATION = '''
---
module: azure_rm_sql_elasticpooldatabaseactivities_facts
version_added: "2.5"
...
EXAMPLES = '''
      - name: List instances of ElasticPoolDatabaseActivities
        azure_rm_sql_elasticpooldatabaseactivities_facts:
...
class AzureRMElasticPoolDatabaseActivitiesFacts(AzureRMModuleBase):
    ...
    def list_by_elastic_pool(self):
        ...
        found = False
        try:
            response = self.mgmt_client.elastic_pool_database_activities.list_by_elastic_pool(self.resource_group_name,
                                                                                              self.server_name,
